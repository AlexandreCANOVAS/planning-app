            ...
            <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                <form action="{{ route('documents.update', $document->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="titre" id="titre" value="{{ old('titre', $document->titre) }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-500 focus:ring-opacity-50">
                                @error('titre')
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-500 focus:ring-opacity-50">{{ old('description', $document->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <div class="mt-1 flex items-center">
                                ...
                                <span class="ml-3 text-sm text-gray-700">{{ basename($document->fichier_path) }}</span>
                            </div>
                        </div>
                        
                        <!-- Nouveau fichier -->
                        <div class="mt-4">
                            <label for="fichier" class="block text-sm font-medium text-gray-700 mb-1">Remplacer le fichier</label>
                        ...
                        <div class="mt-4">
                            ...
                            <input type="date" name="date_expiration" id="date_expiration" value="{{ old('date_expiration', $document->date_expiration ? \Carbon\Carbon::parse($document->date_expiration)->format('Y-m-d') : '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-500 focus:ring-opacity-50">
                    ...
                    <div class="mb-6">
                        ...
                        <div>
                            ...
                            <select name="societe_id" id="societe_id" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-500 focus:ring-opacity-50">
                                ...
                                @foreach($societes as $societe)
                                    <option value="{{ $societe->id }}" {{ old('societe_id', $document->societe_id) == $societe->id ? 'selected' : '' }}>{{ $societe->nom }}</option>
                                @endforeach
                            </select>
                            @error('societe_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mt-4">
                            <div class="flex items-start">
                                <div class="flex items-center h-5">
                                    <input id="visible_pour_tous" name="visible_pour_tous" type="checkbox" {{ old('visible_pour_tous', $document->visible_pour_tous) ? 'checked' : '' }} class="focus:ring-purple-500 h-4 w-4 text-purple-600 border-gray-300 rounded">
                                </div>
                        ...
                        <div id="selection-employes" class="mt-4 {{ old('visible_pour_tous', $document->visible_pour_tous) ? 'hidden' : '' }}">
                    ...
                    <div class="flex justify-end space-x-3">
                        <a href="{{ route('documents.show', $document->id) }}" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded-lg">
                            Annuler
                        </a>

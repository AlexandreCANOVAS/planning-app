<x-app-layout>
        ...
        <div class="relative overflow-hidden bg-gradient-to-r from-blue-600 to-blue-800 rounded-xl shadow-lg p-8">
            ...
            <div class="relative flex justify-between items-center">
                ...
                <div class="hidden md:block">
                    <div class="bg-white bg-opacity-10 rounded-lg px-4 py-2 text-white text-sm backdrop-blur-sm">
                        <i class="fas fa-building mr-2"></i>
                        SIRET : {{ chunk_split($societe->siret, 3, ' ') }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl rounded-2xl">
                <div class="p-8">
                    <form method="POST" action="{{ route('societes.update', $societe) }}" class="space-y-8">
                        @csrf
                        @method('PUT')
                                    ...
                                    <div class="col-span-2 md:col-span-1">
                                        ...
                                        <div class="relative rounded-md shadow-sm">
                                            ...
                                            <input type="text" id="siret" name="siret"
                                                class="block w-full pl-10 pr-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent"
                                                maxlength="19"
                                                value="{{ old('siret', $societe->siret) }}" required>
                                            <input type="hidden" id="siret_hidden" name="siret" value="{{ old('siret', $societe->siret) }}">
                                        </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="relative rounded-md shadow-sm">
                                            ...
                                            <select id="forme_juridique" name="forme_juridique" 
                                                ...
                                                <option value="SARL" {{ old('forme_juridique', $societe->forme_juridique) == 'SARL' ? 'selected' : '' }}>SARL - Société à Responsabilité Limitée</option>
                                                <option value="EURL" {{ old('forme_juridique', $societe->forme_juridique) == 'EURL' ? 'selected' : '' }}>EURL - Entreprise Unipersonnelle à Responsabilité Limitée</option>
                                                <option value="SAS" {{ old('forme_juridique', $societe->forme_juridique) == 'SAS' ? 'selected' : '' }}>SAS - Société par Actions Simplifiée</option>
                                                <option value="SASU" {{ old('forme_juridique', $societe->forme_juridique) == 'SASU' ? 'selected' : '' }}>SASU - Société par Actions Simplifiée Unipersonnelle</option>
                                                <option value="SA" {{ old('forme_juridique', $societe->forme_juridique) == 'SA' ? 'selected' : '' }}>SA - Société Anonyme</option>
                                                <option value="SCI" {{ old('forme_juridique', $societe->forme_juridique) == 'SCI' ? 'selected' : '' }}>SCI - Société Civile Immobilière</option>
                                            </select>
                                    ...
                                    <div>
                                        ...
                                        <div class="relative rounded-md shadow-sm">
                                            ...
                                            <textarea id="adresse" name="adresse" rows="3"
                                                class="block w-full pl-10 pr-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent"
                                                required>{{ old('adresse', $societe->adresse) }}</textarea>
                                        </div>
    ...
    <script>
        ...
        // Formatage initial du SIRET
        window.addEventListener('load', function() {
            const value = siretInput.value.replace(/\D/g, '');
        ...
        // Formatage initial du téléphone
        window.addEventListener('load', function() {
            const value = telInput.value.replace(/\D/g, '');
            telInput.value = value.replace(/(\d{2})(?=\d)/g, '$1 ').trim();
        });

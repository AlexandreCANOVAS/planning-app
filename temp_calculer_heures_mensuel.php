        ...
        foreach ($planningsParSemaine as $semaine => $planningsSemaine) {
            ...
            foreach ($planningsSemaine as $planning) {
                ...
                if (!empty($planning->heure_debut) && !empty($planning->heure_fin)) {
                    // On considère que c'est un service de nuit uniquement si:
                    // - L'heure de début est à 21h ou plus tard
                    // - OU l'heure de début est avant 6h du matin
                    $heureDebut = (int)\Carbon\Carbon::parse($planning->heure_debut)->format('H');
                    
                    if ($heureDebut >= 21 || $heureDebut < 6) {
                        // Si c'est un service de nuit, on compte toutes les heures travaillées comme heures de nuit
                        $heuresNuitSemaine += $heuresTravaillees;
                    }
                }

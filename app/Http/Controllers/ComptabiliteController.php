        ...
        try {
            ...
            while ($date <= $finMoisEtSemaine) {
                ...
                foreach ($planningsSemaine as $planning) {
                    ...
                    if (!empty($planning->heure_debut) && !empty($planning->heure_fin) && $planning->lieu && !in_array($planning->lieu->nom, ['RH', 'CP'])) {
                        // Approche simplifiée pour le calcul des heures de nuit
                        $heureDebut = (int)Carbon::parse($planning->heure_debut)->format('H');
                        $heureFin = (int)Carbon::parse($planning->heure_fin)->format('H');
